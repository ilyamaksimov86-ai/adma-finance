...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(action==='save_lead'){
   ...
   let data,error;if(raw.id)({data,error}=await db.from('leads').update({...value,updated_at:new Date().toISOString()}).eq('id',id(raw.id)).select('*').single());else({data,error}=await db.from('leads').insert({...value,created_by:user.id}).select('*').single());if(error)throw error;let designer=null;if(data.designer_id){const result=await db.from('designers').select('*').eq('id',data.designer_id).single();if(result.error)throw result.error;designer=result.data}return json({ok:true,lead:data,designer});
  }
  if(action==='add_interaction'){
   const value=body.interaction||{},leadId=id(value.lead_id),type=String(value.interaction_type||'note');if(!interactionTypes.has(type))throw new Error('invalid_interaction_type');const {data:lead,error:leadError}=await db.from('leads').select('id').eq('id',leadId).single();if(leadError||!lead)throw leadError||new Error('lead_not_found');const {data,error}=await db.from('lead_interactions').insert({lead_id:leadId,occurred_at:timestamp(value.occurred_at)||new Date().toISOString(),interaction_type:type,comment:text(value.comment,4000,true),created_by:user.id}).select('*').single();if(error)throw error;const updated=await db.from('leads').select('*').eq('id',leadId).single();if(updated.error)throw updated.error;return json({ok:true,interaction:data,lead:updated.data});
  }
  if(action==='convert_lead'){
   const leadId=id(body.lead_id),project=projectInput(body.project||{});const {data,error}=await db.rpc('convert_lead_to_project',{p_lead_id:leadId,p_project:project,p_actor_id:user.id});if(error)throw error;const leadResult=await db.from('leads').select('*').eq('id',leadId).single();if(leadResult.error)throw leadResult.error;let designer=null;if(leadResult.data.designer_id){const designerResult=await db.from('designers').select('*').eq('id',leadResult.data.designer_id).single();if(designerResult.error)throw designerResult.error;designer=designerResult.data}return json({ok:true,project:data,lead:leadResult.data,designer});
  }
