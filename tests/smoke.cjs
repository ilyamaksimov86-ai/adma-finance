...
(async () => {
  ...
  try {
    ...
    let expenses = [], acts = [], actCosts = [], actPayments = [], waybills = [], waybillPayments = [], companyExpenses = [], masters = [], masterAssignments = [], projectDocuments = [], projectTasks = [], projectPhotos = [], failSave = false, delaySave = false;
    ...
    await page.route('https://blaacuwwvyatfiyjnsrw.supabase.co/**', async route => {
      ...
      if (url.endsWith('/receipt-upload')) {
        ...
        if((url.endsWith('/adma-api')||url.endsWith('/finance-api')||url.endsWith('/masters-api')||url.endsWith('/project-operations-api')) && web) assert.equal(body.accessToken,'test-token');
