...
(async () => {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!['index.html','app.js','cloud.js','styles.css'].includes(name)) { res.writeHead(404).end(); return; }
  ...
  try {
    ...
    await page.addInitScript(() => {
      window.Telegram = {WebApp:{initData:'test-only',ready(){},expand(){},openLink(url){window.lastPdf=url;}}};
    ...
    await page.route('https://blaacuwwvyatfiyjnsrw.supabase.co/**', async route => {
      ...
      if (url.endsWith('/receipt-upload')) {
        ...
        const body=req.postDataBuffer().toString(); pdfs.push(body);
        ...
        else if(body.action==='load') data={ok:true,projects,expenses};
