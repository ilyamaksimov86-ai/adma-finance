...
(async () => {
  ...
  try {
    ...
    const requests = [], uploads = [], projectUploads = [], pdfs = [];
    ...
    let expenses = [], acts = [], actCosts = [], actPayments = [], waybills = [], waybillPayments = [], companyExpenses = [], masters = [], masterAssignments = [], projectDocuments = [], projectTasks = [], projectPhotos = [], designers = [], designerInteractions = [], failSave = false, delaySave = false;
    ...
    await page.route('https://blaacuwwvyatfiyjnsrw.supabase.co/**', async route => {
      ...
      if (url.endsWith('/receipt-upload')) {
        ...
        if((url.endsWith('/adma-api')||url.endsWith('/finance-api')||url.endsWith('/masters-api')||url.endsWith('/project-operations-api')||url.endsWith('/designers-api')) && web) assert.equal(body.accessToken,'test-token');
        if(url.endsWith('/telegram-auth')) data.user={id:'user',role:'owner',is_active:true};
          ...
          if(duplicate&&!body.allow_duplicate){status=409;data={error:'possible_duplicate'};}else if(d.id)Object.assign(designers.find(x=>x.id===d.id),d,{updated_at:new Date().toISOString()});else designers.push({...d,id:'designer-'+(designers.length+1),created_at:new Date().toISOString(),is_archived:false});
        }
        else if(url.endsWith('/designers-api')&&body.action==='archive_designer'){const d=designers.find(x=>x.id===body.id);Object.assign(d,{is_archived:body.archived!==false,status:body.archived!==false?'inactive':'found'});}
        else if(url.endsWith('/designers-api')&&body.action==='add_interaction'){const d=designers.find(x=>x.id===body.interaction.designer_id),item={...body.interaction,id:'interaction-'+(designerInteractions.length+1),author:{first_name:'Илья'}};designerInteractions.push(item);if(['message','call','meeting'].includes(item.interaction_type))d.last_contact_at=item.occurred_at;}
        ...
        else if(url.endsWith('/project-operations-api')&&body.action==='save_document')Object.assign(projectDocuments.find(x=>x.id===body.document.id),body.document);
        ...
        else if(url.endsWith('/project-operations-api')&&body.action==='save_task'){if(body.task.id)Object.assign(projectTasks.find(x=>x.id===body.task.id),body.task);else projectTasks.push({...body.task,id:'task-'+(projectTasks.length+1),created_at:new Date().toISOString(),author:{first_name:'Илья'}});}
        else if(url.endsWith('/project-operations-api')&&body.action==='delete_task')projectTasks=projectTasks.filter(x=>x.id!==body.id);
        else if(url.endsWith('/project-operations-api')&&body.action==='save_photo')Object.assign(projectPhotos.find(x=>x.id===body.photo.id),body.photo);
