...
(async () => {
  ...
  try {
    ...
    await switchTab('masters');await page.click('#addMaster');await page.fill('#masterEditDlg [name="name"]','Иван Мастер');await page.fill('#masterEditDlg [name="phone"]','+7 900 111-22-33');await page.fill('#masterEditDlg [name="telegram"]','@ivanmaster');await page.selectOption('#masterEditDlg [name="specialty"]','electrical');await page.selectOption('#masterEditDlg [name="skills"]',['rough','universal']);await page.fill('#masterEditDlg [name="rating"]','4.7');await page.selectOption('#masterEditDlg [name="price"]','high');await page.locator('#masterEditDlg form').evaluate(f=>f.requestSubmit());await until(()=>masters.length===1);assert.equal(await page.locator('[data-master-id]').count(),1);
