...
(() => {
  ...
  function filteredLeads(){return(state.leads||[]).filter(x=>{const search=[x.clientName,x.phone,x.telegram,x.projectName,x.address].join(' ').toLowerCase(),created=(x.createdAt||'').slice(0,10);return(!leadSearch||search.includes(leadSearch.toLowerCase()))&&(!leadStatusFilter||x.status===leadStatusFilter)&&(!leadSourceFilter||x.source===leadSourceFilter)&&(!leadResponsibleFilter||x.responsibleUserId===leadResponsibleFilter)&&(!leadDesignerFilter||x.designerId===leadDesignerFilter)&&(!leadContactFilter||leadContactFilter==='overdue'&&leadOverdue(x))&&(!leadCreatedFrom||created>=leadCreatedFrom)&&(!leadCreatedTo||created<=leadCreatedTo)})}
  const leadResultCard=x=>`<button class="card designer-card lead-card ${leadOverdue(x)?'overdue':''}" data-lead-id="${esc(x.id)}"><span class="lead-avatar">◇</span><span class="grow"><strong>${esc(x.clientName)} · ${esc(x.projectName)}</strong><small>${x.area?esc(x.area+' м² · '):''}${x.budget!=null?money(x.budget)+' · ':''}${esc(leadSourceLabels[x.source]||x.source)}${x.designerId?' · '+esc(designerName(leadDesigner(x))):''}</small><small>${esc(x.nextAction||'Следующее действие не задано')}</small></span><span><span class="badge ${leadOverdue(x)?'danger':x.status==='contract'?'paid':x.status==='lost'?'neutral':'pending'}">${esc(leadStatusLabels[x.status]||x.status)}</span><small class="designer-next ${leadOverdue(x)?'danger-text':''}">${esc(x.nextContactAt?designerContactDate(x.nextContactAt):'Без даты')}</small></span></button>`;
  function renderLeadResults(){const root=document.getElementById('leadResults');if(!root)return;const oldBoard=root.querySelector('.crm-funnel-board'),scrollLeft=oldBoard?.scrollLeft||0,filtered=filteredLeads();root.innerHTML=leadView==='funnel'?`<div class="designer-funnel lead-funnel crm-funnel-board">${Object.entries(leadStatusLabels).map(([v,l])=>{const items=filtered.filter(x=>x.status===v);return `<section class="designer-column"><div class="section compact"><h3>${esc(l)}</h3><span class="badge neutral">${items.length}</span></div>${items.map(leadResultCard).join('')||'<div class="empty">Пусто</div>'}</section>`}).join('')}</div>`:`<div class="designer-list">${filtered.map(leadResultCard).join('')||'<div class="card empty">Заявки по фильтру не найдены</div>'}</div>`;const board=root.querySelector('.crm-funnel-board');if(board)board.scrollLeft=scrollLeft;root.querySelectorAll('[data-lead-id]').forEach(b=>b.onclick=()=>openLeadDetails(b.dataset.leadId));document.getElementById('leadListView').className=`btn ${leadView==='list'?'primary':'secondary'}`;document.getElementById('leadFunnelView').className=`btn ${leadView==='funnel'?'primary':'secondary'}`}
  function renderLeadsCloudHotfix(){if(!canManageProjects()){$('#app').innerHTML=`${pageHeader('ADMA · ЗАЯВКИ','Заявки','CRM доступна владельцу и партнёру')}${moduleScreen('◇','Доступ ограничен','Работа с заявками доступна владельцу и партнёру.','Дизайнер → Заявка → Объект')}`;return}
    $('#app').innerHTML=`${pageHeader('ADMA · ПРОДАЖИ','Заявки','Воронка обращений и конвертация в объекты','<button id="addLead" class="btn primary">+ Заявка</button>')}<section class="card lead-filters"><label>Поиск<input id="leadSearch" type="search" placeholder="Клиент, телефон, ЖК" value="${esc(leadSearch)}"></label><label>Статус<select id="leadStatus"><option value="">Все</option>${Object.entries(leadStatusLabels).map(([v,l])=>`<option value="${v}" ${leadStatusFilter===v?'selected':''}>${l}</option>`).join('')}</select></label><label>Источник<select id="leadSource"><option value="">Все</option>${Object.entries(leadSourceLabels).map(([v,l])=>`<option value="${v}" ${leadSourceFilter===v?'selected':''}>${l}</option>`).join('')}</select></label><label>Ответственный<select id="leadResponsible"><option value="">Все</option>${(state.leadUsers||[]).map(u=>`<option value="${esc(u.id)}" ${leadResponsibleFilter===u.id?'selected':''}>${esc(responsibleName(u))}</option>`).join('')}</select></label><label>Дизайнер<select id="leadDesigner"><option value="">Все</option>${(state.designers||[]).filter(d=>!d.isArchived).map(d=>`<option value="${esc(d.id)}" ${leadDesignerFilter===d.id?'selected':''}>${esc(designerName(d))}</option>`).join('')}</select></label><label>Контакт<select id="leadContact"><option value="">Любой</option><option value="overdue" ${leadContactFilter==='overdue'?'selected':''}>Просрочен</option></select></label><label>Создана с<input id="leadFrom" type="date" value="${esc(leadCreatedFrom)}"></label><label>по<input id="leadTo" type="date" value="${esc(leadCreatedTo)}"></label><div class="designer-view-buttons"><button id="leadListView" class="btn">Список</button><button id="leadFunnelView" class="btn">Воронка</button></div></section><div id="leadResults"></div>`;
    document.getElementById('addLead').onclick=()=>openLeadDialog();const binds=[['leadSearch','input',v=>leadSearch=v],['leadStatus','change',v=>leadStatusFilter=v],['leadSource','change',v=>leadSourceFilter=v],['leadResponsible','change',v=>leadResponsibleFilter=v],['leadDesigner','change',v=>leadDesignerFilter=v],['leadContact','change',v=>leadContactFilter=v],['leadFrom','change',v=>leadCreatedFrom=v],['leadTo','change',v=>leadCreatedTo=v]];binds.forEach(([id,event,set])=>document.getElementById(id).addEventListener(event,e=>{set(e.target.value);renderLeadResults()}));document.getElementById('leadListView').onclick=()=>{leadView='list';renderLeadResults()};document.getElementById('leadFunnelView').onclick=()=>{leadView='funnel';renderLeadResults()};renderLeadResults()
  }
  ...
  function filteredKnowledge(){const tech=knowledgeSection==='tech',source=tech?(state.knowledgeTechCards||[]):(state.knowledgeIssues||[]),query=knowledgeSearch.toLocaleLowerCase('ru');return source.filter(item=>{if(knowledgeCategory&&item.category!==knowledgeCategory)return false;const searchable=tech?[item.title,item.category,item.description,...knowledgeItemsFor(item.id).map(x=>x.text)].join(' '):[item.title,item.category,item.problem,item.cause,item.solution,item.prevention].join(' ');return!query||searchable.toLocaleLowerCase('ru').includes(query)})}
  function renderKnowledgeResults(){const root=document.getElementById('knowledgeResults');if(!root)return;const tech=knowledgeSection==='tech',items=filteredKnowledge();root.innerHTML=items.map(item=>tech?`<button class="card knowledge-card" data-tech-card="${esc(item.id)}"><span class="knowledge-card-icon">▤</span><span class="grow"><span class="badge neutral">${esc(item.category)}</span><strong>${esc(item.title)}</strong><small>${esc(knowledgeExcerpt(item.description)||'Описание не добавлено')}</small><span class="knowledge-card-meta">${knowledgeItemsFor(item.id).length} пунктов · обновлено ${knowledgeDate(item.updatedAt)}</span></span></button>`:`<button class="card knowledge-card issue" data-issue="${esc(item.id)}"><span class="knowledge-card-icon">!</span><span class="grow"><span class="badge pending">${esc(item.category)}</span><strong>${esc(item.title)}</strong><small>${esc(knowledgeExcerpt(item.problem)||'Описание проблемы не добавлено')}</small><span class="knowledge-card-meta">Обновлено ${knowledgeDate(item.updatedAt)}</span></span></button>`).join('')||`<div class="card empty">${tech?'Техкарт':'Косяков'} по выбранным условиям пока нет</div>`;document.getElementById('knowledgeCount').textContent=`${items.length} записей`;root.querySelectorAll('[data-tech-card]').forEach(button=>button.onclick=()=>openTechCardDialog(button.dataset.techCard));root.querySelectorAll('[data-issue]').forEach(button=>button.onclick=()=>openKnowledgeIssueDialog(button.dataset.issue))}
  function renderKnowledgeCloudHotfix(){const action=knowledgeSection==='tech'?'+ Добавить техкарту':'+ Добавить косяк';$('#app').innerHTML=`${pageHeader('ADMA · ОПЫТ','База знаний','Технологии и решения ADMA',`<button id="addKnowledge" class="btn primary">${action}</button>`)}${sectionTabs([['tech','Техкарты'],['issues','Косяки']],knowledgeSection,'data-knowledge-section')}<section class="knowledge-filters"><label>Поиск<input id="knowledgeSearch" type="search" value="${esc(knowledgeSearch)}" placeholder="Название, этап или содержание"></label><label>Этап / категория<select id="knowledgeCategory"><option value="">Все категории</option>${knowledgeCategories().map(x=>`<option value="${esc(x)}" ${knowledgeCategory===x?'selected':''}>${esc(x)}</option>`).join('')}</select></label><span id="knowledgeCount"></span></section><div id="knowledgeResults" class="knowledge-list"></div>`;document.querySelectorAll('[data-knowledge-section]').forEach(button=>button.onclick=()=>{knowledgeSection=button.dataset.knowledgeSection;knowledgeSearch='';knowledgeCategory='';document.getElementById('knowledgeSearch').value='';document.getElementById('knowledgeCategory').value='';document.querySelectorAll('[data-knowledge-section]').forEach(tab=>tab.classList.toggle('active',tab.dataset.knowledgeSection===knowledgeSection));document.getElementById('addKnowledge').textContent=knowledgeSection==='tech'?'+ Добавить техкарту':'+ Добавить косяк';renderKnowledgeResults()});document.getElementById('knowledgeSearch').oninput=event=>{knowledgeSearch=event.target.value;renderKnowledgeResults()};document.getElementById('knowledgeCategory').onchange=event=>{knowledgeCategory=event.target.value;renderKnowledgeResults()};document.getElementById('addKnowledge').onclick=()=>knowledgeSection==='tech'?openTechCardDialog():openKnowledgeIssueDialog();renderKnowledgeResults()}
  ...
  function renderMastersCloud(){
    ...
    $('#app').innerHTML=`${pageHeader('ADMA · КОМАНДА','Мастера','Единая база специалистов и занятость по объектам','<button id="addMaster" class="btn primary">+ Мастер</button>')}<section class="card master-filters"><label>Поиск<input id="masterSearch" type="search" placeholder="Имя, телефон, Telegram" value="${esc(masterSearch)}"></label><label>Специализация<select id="masterSpecialty"><option value="">Все специализации</option>${Object.entries(masterSpecialtyLabels).map(([value,label])=>`<option value="${value}" ${masterSpecialtyFilter===value?'selected':''}>${label}</option>`).join('')}</select></label><label>Занятость<select id="masterAvailability"><option value="">Любая</option><option value="free" ${masterAvailabilityFilter==='free'?'selected':''}>Свободен</option><option value="releasing" ${masterAvailabilityFilter==='releasing'?'selected':''}>Есть дата освобождения</option><option value="busy" ${masterAvailabilityFilter==='busy'?'selected':''}>Занят без даты</option></select></label><button id="toggleArchivedMasters" class="btn secondary">${showArchivedMasters?'Активные':'Архив'}</button></section><div id="masterResults" class="master-list"></div>`;
    document.getElementById('addMaster').onclick=()=>openMasterDialog();document.getElementById('masterSearch').oninput=e=>{masterSearch=e.target.value;renderMasterResults()};document.getElementById('masterSpecialty').onchange=e=>{masterSpecialtyFilter=e.target.value;renderMasterResults()};document.getElementById('masterAvailability').onchange=e=>{masterAvailabilityFilter=e.target.value;renderMasterResults()};document.getElementById('toggleArchivedMasters').onclick=()=>{showArchivedMasters=!showArchivedMasters;document.getElementById('toggleArchivedMasters').textContent=showArchivedMasters?'Активные':'Архив';renderMasterResults()};renderMasterResults();
  }
  ...
  function filteredDesigners(){const today=new Date().toISOString().slice(0,10),all=(state.designers||[]).filter(d=>showArchivedDesigners?d.isArchived:!d.isArchived);return all.filter(d=>{const search=[d.fullName,d.studio,d.instagram,d.telegram,d.phone,d.email,d.city,...(d.tags||[])].join(' ').toLowerCase(),day=d.nextContactAt?d.nextContactAt.slice(0,10):'',contactOk=!designerContactFilter||(designerContactFilter==='overdue'?designerOverdue(d):designerContactFilter==='today'?day===today:designerContactFilter==='planned'?day>=today:false);return(!designerSearch||search.includes(designerSearch.toLowerCase()))&&(!designerStatusFilter||d.status===designerStatusFilter)&&(!designerResponsibleFilter||d.responsibleUserId===designerResponsibleFilter)&&contactOk})}
  const designerResultCard=d=>`<button class="card designer-card ${designerOverdue(d)?'overdue':''}" data-designer-id="${esc(d.id)}"><span class="master-avatar">${esc(designerName(d).slice(0,2).toUpperCase())}</span><span class="grow"><strong>${esc(designerName(d))}</strong><small>${esc(d.studio||d.city||'Без студии')} · ${esc(d.instagram||d.telegram||d.phone||'контакт не указан')}</small><small>${esc(d.nextAction||'Следующее действие не задано')}</small></span><span><span class="badge ${designerStatusClass(d)}">${esc(designerStatusLabels[d.status]||d.status)}</span><small class="designer-next ${designerOverdue(d)?'danger-text':''}">${esc(d.nextContactAt?designerContactDate(d.nextContactAt):'Без даты')}</small></span></button>`;
  function renderDesignerResults(){const root=document.getElementById('designerResults');if(!root)return;const oldBoard=root.querySelector('.crm-funnel-board'),scrollLeft=oldBoard?.scrollLeft||0,filtered=filteredDesigners();root.innerHTML=designerView==='funnel'?`<div class="designer-funnel crm-funnel-board">${Object.entries(designerStatusLabels).map(([v,l])=>{const items=filtered.filter(d=>d.status===v);return `<section class="designer-column"><div class="section compact"><h3>${esc(l)}</h3><span class="badge neutral">${items.length}</span></div>${items.map(designerResultCard).join('')||'<div class="empty">Пусто</div>'}</section>`}).join('')}</div>`:`<div class="designer-list">${filtered.map(designerResultCard).join('')||'<div class="card empty">Дизайнеры по фильтру не найдены</div>'}</div>`;const board=root.querySelector('.crm-funnel-board');if(board)board.scrollLeft=scrollLeft;root.querySelectorAll('[data-designer-id]').forEach(b=>b.onclick=()=>openDesignerDetails(b.dataset.designerId));document.getElementById('designerListView').className=`btn ${designerView==='list'?'primary':'secondary'}`;document.getElementById('designerFunnelView').className=`btn ${designerView==='funnel'?'primary':'secondary'}`}
